...
export function eventBumpHtml(params: {
  eventName: string
  groupName: string
  startTime: string
  endTime: string
  ...
  const { eventName, groupName, startTime, endTime, location, description, customMessage, rsvpUrl, hosts } = params
  const descHtml = renderMarkdown(description)
  const customHtml = renderMarkdown(customMessage)
